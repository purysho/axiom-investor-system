  ...
  return (
    <div>
      <section className="mb-10 rounded-[26px] bg-panel p-6 sm:p-9">
        <Sparkles size={25} className="text-faint" />
      ...
      <section className="mb-12">
        ...
        <div className="mt-6 grid gap-5 border-y border-line py-6 sm:grid-cols-3">
      ...
      <section className="mb-12 border-t border-line pt-10">
        ...
        <div className="mt-7 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          <div className="border-l-2 border-line pl-5"><BarChart3 size={18} className="text-faint" /><div className="mt-3 text-xs font-semibold text-faint">Closed trades</div><div className="mt-1 text-2xl font-semibold">{js.closedCount}</div></div>
          <div className="border-l-2 border-line pl-5"><Scale size={18} className="text-faint" /><div className="mt-3 text-xs font-semibold text-faint">Average R</div><div className="mt-1 text-2xl font-semibold">{fmtR(js.avgR)}</div></div>
          ...
          <div className="border-l-2 border-line pl-5"><CircleCheckBig size={18} className="text-faint" /><div className="mt-3 text-xs font-semibold text-faint">Rule adherence</div><div className="mt-1 text-2xl font-semibold">{fmtPct(js.adherencePct, 0)}</div></div>
        </div>
        {js.mistakeCounts.length > 0 && <div className="mt-7 rounded-[18px] bg-panel2/75 p-5"><div className="font-semibold">Repeated journal tags</div><p className="mt-2 text-sm leading-relaxed text-mut">{js.mistakeCounts.map(([tag, n]) => `${tag} ×${n}`).join(" · ")}. A repeated tag is a question to investigate, not an automatic new rule.</p></div>}
      </section>
      ...
      <section className="mb-12 border-t border-line pt-10">
        ...
        <label className="mt-6 flex cursor-pointer items-start gap-3 rounded-[18px] bg-panel2/70 p-5">
        ...
        {draft.changeRule && (
          <div className="mt-5 rounded-[22px] bg-panel2/76 p-6 sm:p-8">
                ...
                return (
                  <details key={m.id} className="group">
                    <summary className="grid cursor-pointer list-none gap-4 py-5 sm:grid-cols-[1fr_auto_auto] sm:items-center">
                      ...
                      <div className="flex items-center gap-1 text-sm font-semibold text-faint">Read review <ChevronDown size={15} className="transition-transform group-open:rotate-180" /></div>
                    </summary>
