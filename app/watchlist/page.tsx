...
export default function WatchlistPage() {
  ...
  return (
    <div>
      <section className={`mb-10 rounded-[26px] p-6 sm:p-9 ${locked ? "bg-[#241111]" : "bg-panel"}`}>
        {locked ? <LockKeyhole size={25} className="text-[#F4645C]" /> : <Lightbulb size={25} className="text-faint" />}
          ...
          <div className="mt-5 divide-y divide-line border-y border-line">
            {firedAlerts.map((a) => (
              <div key={a.rule.id} className="grid gap-3 py-5 sm:grid-cols-[44px_1fr_auto] sm:items-center">
                <span className="flex h-10 w-10 items-center justify-center rounded-full bg-panel2 text-[#F0B429]"><Bell size={18} /></span>
        ...
        {state.watchRules.length === 0 ? (
          <div className="border-y border-line py-8">
            <div className="max-w-2xl">
              <CircleHelp size={22} className="text-faint" />
            ...
            {state.watchRules.map((rule) => {
              ...
              return (
                <div key={rule.id} className="grid gap-4 py-5 sm:grid-cols-[1fr_auto] sm:items-center">
                  <div>
                    <div className="flex flex-wrap items-center gap-3"><span className="text-[18px] font-bold">{rule.ticker}</span>{alert?.fired && <span className="badge bg-panel2 text-[#F0B429]">Condition met</span>}</div>
