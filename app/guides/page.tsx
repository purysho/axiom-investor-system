...
export default function GuidesPage() {
  return (
    <div>
      <section className="mb-12 rounded-[26px] bg-panel p-6 sm:p-9">
        <Lightbulb size={25} className="text-faint" />
      ...
      <section className="mb-12">
        ...
        <div className="mt-6 grid gap-7 md:grid-cols-3">
          ...
          ].map(([n, title, text]) => <div key={n} className="border-t border-line pt-5"><div className="text-sm font-bold text-[#a16b4c]">{n}</div><h3 className="mt-3 font-display text-2xl font-semibold tracking-[-0.025em]">{title}</h3><p className="mt-2 text-sm leading-relaxed text-mut">{text}</p></div>)}
        </div>
      </section>
          ...
          {LESSONS.map((lesson) => (
            <details key={lesson.id} id={lesson.id} className="group scroll-mt-28">
              <summary className="grid cursor-pointer list-none gap-4 py-6 sm:grid-cols-[48px_1fr_auto] sm:items-start">
                <span className="flex h-11 w-11 items-center justify-center rounded-full bg-panel text-faint"><lesson.Icon size={19} /></span>
                ...
                <span className="flex items-center gap-1 text-sm font-semibold text-faint">Read <ChevronDown size={15} className="transition-transform group-open:rotate-180" /></span>
              </summary>
              <div className="mb-7 ml-0 rounded-[22px] bg-panel2/76 p-6 sm:ml-16 sm:p-8">
